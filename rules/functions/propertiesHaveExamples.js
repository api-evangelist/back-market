/**
 ...
 module.exports =(item, _, paths) => {
    if (
       (paths.target && paths.target.join('.').includes('properties.properties')) ||
        // skip if this is an example
        (paths.target && paths.target.includes('example')) ||
    ...
    if (!item.example && item.example !== false) {
      return [
        {
          message: `${paths.target ? paths.target.join('.') : 'property'} does not have an example`,
        }
      ]
    }
}